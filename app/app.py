    ...
    # ---------------------------------------------------------------
        ...
        st.markdown(
            ...
            **Metodologia:** validação temporal *out-of-time* — o modelo é treinado com a
            ...
            O alvo é: o aluno **piorou a defasagem** no ano seguinte **ou** já **permanece em
            situação crítica** (`Defasagem ≤ -1`).
            """
        )
        if AUC_TESTE:
            col1, col2 = st.columns(2)
            col1.metric("AUC — Random Forest (usado no app)", f"{AUC_TESTE:.3f}")
            col2.metric("AUC — Regressão Logística", "0.683")
        ...
        st.markdown(
            ...
            "validação out-of-time, indicando que a relação entre os indicadores pedagógicos e o "
            "risco futuro é não linear. IAN, IDA e IEG concentram a maior parte do sinal "
            "preditivo. É uma ferramenta estatisticamente válida para apoiar a priorização de "
...
# =========================================================================
    ...
    st.markdown(
        ...
        piorar a defasagem (ou permanecer em situação crítica) no próximo ciclo**. O modelo foi
        validado de forma temporal (treino 2022→2023, teste 2023→2024 — dados "do futuro" nunca
        entram no treino).
        """
    )
    if AUC_TESTE:
        st.caption(f"Desempenho do modelo na validação out-of-time: AUC = {AUC_TESTE:.3f} (Random Forest)")

    st.markdown("---")

    valores_default = {"IAN": 6.9, "IDA": 6.7, "IEG": 8.6, "IAA": 7.7, "IPS": 6.0, "IPP": 7.5}

    col1, col2 = st.columns(2)
    entrada = {}
    campos = list(FEATURES)
...
# =========================================================================
    ...
    with st.form("form_cadastro", clear_on_submit=False):
        ...
        with c9:
            ...
            ian = st.slider("IAN", 0.0, 10.0, 6.9, 0.1)
