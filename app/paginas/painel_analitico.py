...
def _pergunta_4(df):
    ...
    st.markdown(
        ...
        "Vale a pena investigar isso qualitativamente: pode ser efeito de autoestima elevada mesmo com dificuldades, ou um instrumento de autoavaliação pouco assertivo."
    )
...
def _pergunta_8(df, modelo_pack):
    ...
    st.markdown(
        "A ponderação abaixo é a **fórmula oficial do INDE**, publicada pela própria Passos "
        "Mágicos no material metodológico do PEDE (quadro \"Composição do Índice de "
        "Desenvolvimento Educacional\"). Ela muda conforme a fase do aluno:"
    )

    pesos_0_7 = {"IAN": 0.10, "IDA": 0.20, "IEG": 0.20, "IAA": 0.10, "IPS": 0.10, "IPP": 0.10, "IPV": 0.20}

    col_f1, col_f2 = st.columns(2)
    with col_f1:
        st.caption("Fases 0 a 7")
        st.code(
            "INDE = 0,10×IAN + 0,20×IDA + 0,20×IEG + 0,10×IAA\n"
            "     + 0,10×IPS + 0,10×IPP + 0,20×IPV",
            language=None,
        )
    with col_f2:
        st.caption("Fase 8 (sem avaliação de IPP/IPV)")
        st.code(
            "INDE = 0,10×IAN + 0,40×IDA + 0,20×IEG\n"
            "     + 0,10×IAA + 0,20×IPS",
            language=None,
        )

    cols_necessarias = list(pesos_0_7.keys()) + ["INDE"]
    df_verif = df[df["Fase_Num"] != 8].dropna(subset=cols_necessarias).copy()
    df_verif["INDE_calculado"] = sum(df_verif[c] * p for c, p in pesos_0_7.items())
    erro_medio = (df_verif["INDE_calculado"] - df_verif["INDE"]).abs().mean()
    n_fase8 = int((df["Fase_Num"] == 8).sum())
    st.caption(
        f"Fórmula das Fases 0-7 conferida em {len(df_verif):,} registros da nossa base — erro "
        f"médio de {erro_medio:.3f} ponto (escala 0-10), compatível com arredondamento. A fórmula "
        f"da Fase 8 não pôde ser conferida: os {n_fase8} alunos nessa fase na nossa base não têm "
        "INDE nem os demais indicadores preenchidos.".replace(",", ".")
    )
    st.markdown(
        "**Leitura:** por definição — não por observação estatística — desempenho acadêmico "
        "(IDA), engajamento (IEG) e ponto de virada (IPV) somam 60% do peso nas Fases 0-7. Na "
        "Fase 8, o peso do IDA sobe para 40% e IPP/IPV saem do cálculo, concentrando a nota em "
        "desempenho, engajamento, autoavaliação e aspectos psicossociais."
    )
    st.markdown("---")
